...
void skills1() {
  ...
  pros::Task antiJamHooks([&]() {
    while (true) {
      ...
      while (hooks.get_target_velocity() - hooks.get_actual_velocity() >
                 hooks.get_target_velocity() * .4 &&
             (hooks.get_torque() > 0.55f)) {
...
=======
  ...
  chassis.moveToPoint(16.5, 15.5, 850,
                      {.forwards = false, .maxSpeed = 60, .earlyExitRange = 2},
                      false);
  chassis.moveToPoint(20, 15.5, 650, {.forwards = false, .maxSpeed = 90}, true);
...
>>>>>>> 252dc33977fe015358a5a093025069c8273f7873
  ...
  chassis.moveToPose(70, 28, 90, 1350, {.minSpeed = 80, .earlyExitRange = 1},
                     false);
...
>>>>>>> 252dc33977fe015358a5a093025069c8273f7873
  ...
  chassis.moveToPoint(chassis.getPose().x - 24, chassis.getPose().y, 2000,
                      {.maxSpeed = 50}, false);
  chassis.waitUntilDone();
